...
const allowedLinks = computed(() =>
  topLinks.value.filter((link) => {
    return !link.forRoles
      ? true
      : link.forRoles.some((role) => userManager.hasRole(userManager.me, role))
  })
)
...
const topLinks = ref<ListItemType[]>([
  ...
  },
  {
    ...
    child: [
      ...
      { icon: '', title: 'مشاهده نمرات', route: { name: 'Panel.inPersonExamList.List' } }
    ]
  },
  {
    icon: 'quiz',
    ...
    forRoles: ['Manager', 'Teacher', 'Admin'],
    child: [
      { icon: '', title: 'ثبت آزمون', route: { name: 'Panel.OnlineExam.Create' } },
      { icon: '', title: 'مشاهده آزمون ها', route: { name: 'Panel.OnlineExam.List' } }
    ]
  },
  {
    icon: 'assignment',
  ...
  },
  {
    ...
    child: [
      ...
      {
        icon: '',
        title: 'مشاهده غیبت های ثبت شده',
        route: { name: 'Panel.Disciplinary.Absence.List' }
      },
