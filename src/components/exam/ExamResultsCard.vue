<template>
  <q-card class="q-mb-md">
    <q-card-section>
      <div class="text-h6">
        {{ delivery_mode === 'in_person' ? 'نمرات ثبت شده' : 'جلسات آزمون آنلاین' }}
      </div>
    </q-card-section>
    <q-separator />
    <q-card-section>
      <template v-if="delivery_mode === 'in_person'">
        <q-list
          v-if="in_person_results?.length"
          bordered
          separator>
          <q-item
            v-for="result in in_person_results"
            :key="result.id"
            class="q-py-sm">
            <q-item-section>
              <q-item-label>{{ result.student?.full_name || '-' }}</q-item-label>
      ...
      <template v-else-if="delivery_mode === 'online'">
        <q-list
          v-if="online_exam_sessions?.length"
          bordered
          separator>
          <q-expansion-item
            v-for="session in online_exam_sessions"
            :key="session.id"
            dense-toggle
            ...
            <template #header>
              <q-item-section>
                ...
                <q-item-label caption>
                  وضعیت: {{ session.status || '-' }} | نمره: {{ session.score ?? '-' }} |
                  درصد: {{ session.percent ?? '-' }}% | تلاش: {{ session.attempt_number ?? '-' }}
                </q-item-label>
              </q-item-section>
              <q-item-section side>
                <q-chip
              ...
              <q-list
                ...
                <q-item
                  ...
                  <q-item-section>
                    ...
                    <q-item-label caption>
                      پاسخ: {{ response.submitted_option || response.answer_text || '-' }} |
                      درست: {{ response.is_correct ? 'بله' : 'خیر' }} |
                      نمره: {{ response.marks_obtained ?? '-' }}
                    </q-item-label>
                  </q-item-section>
                </q-item>
              </q-list>
...
const props = defineProps<{
  ...
  examId: number
  in_person_results?: any[] | null
  online_exam_sessions?: any[] | null
}>()
