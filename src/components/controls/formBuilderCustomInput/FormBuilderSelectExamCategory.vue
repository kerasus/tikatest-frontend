      ...
      <q-item
        ...
        <q-item-section>
          ...
          <q-item-label caption>
            <template v-if="opt.term_number"> ترم {{ opt.term_number }} </template>
            <template v-else> عمومی </template>
          </q-item-label>
        </q-item-section>
      </q-item>
    </template>
...
defineOptions({
  name: 'FormBuilderSelectExamCategory'
})

const props = defineProps({
  label: {
    default: 'دسته‌بندی آزمون',
    type: String
  },
  name: {
    default: '',
...
const filteredOptions = ref<ExamCategoryType[]>([])
